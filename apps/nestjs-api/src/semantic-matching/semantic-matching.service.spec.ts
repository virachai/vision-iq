...
describe("SemanticMatchingService", () => {
	...
	describe("Ranking Formula", () => {
		...
		it("should apply soft mood consistency penalty for non-anchor scenes", async () => {
			...
			// Mock candidates: return same candidate for both scenes for simplicity
			// But we need to ensure the first scene ESTABLISHES the anchor.
			// The service uses the FIRST MATCH of the FIRST SCENE as anchor.
			// So we need to make sure scene 1 returns a match with 'anchorMood'.
			...
			jest
				.spyOn(prismaClient, "$queryRawUnsafe")
				.mockResolvedValueOnce([anchorCandidate]) // For scene 1
				.mockResolvedValueOnce(mockCandidates); // For scene 2
