...
graph TD
    ...
    AppModule --> PrismaModule["PrismaModule<br/>🌐 @Global"]
...
```mermaid
flowchart LR
    A["📝 Raw Text<br/>(Gemini Live)"] --> B["🧠 Intent Extraction<br/>(DeepSeek)"]
    B --> C["📋 Description Expansion<br/>(DeepSeek)"]
    C --> D["🔑 Keyword Extraction<br/>(DeepSeek)"]
    D --> E["📸 Image Sync<br/>(Pexels API)"]
    E --> F["🔬 Image Analysis<br/>(Gemini Vision)"]
    F --> G["🎯 Semantic Matching<br/>(pgvector)"]
```
...
flowchart TD
    ...
    W2 --> |⚠️ disabled| DB2[Placeholder: Math.sin]
